...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const restaurant = formatData(data, category);
    ...
    await prisma.restaurantV2.create({
      data: {
        ...
        categories: {
          connectOrCreate: restaurant.categories.map((category: any) => {
            return {
              where: { name: category },
...
export function formatData(data: any, category: string) {
  // return jsonArray.map((item: any) => {
  const { opening_hours, geometry, address_components, ...rest } = data.result;
